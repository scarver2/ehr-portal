...
RSpec.describe Patient, type: :model do
  ...
  describe "#age" do
    ...
    it "calculates age from date_of_birth" do
      patient.date_of_birth = Date.new(Date.today.year - 30, 1, 1)
  ...
  describe ".search_by_name" do
    ...
    it "finds by first name prefix" do
      expect(Patient.search_by_name("jan")).to include(jane)
      expect(Patient.search_by_name("jan")).not_to include(john)
    end

    it "finds by last name" do
      expect(Patient.search_by_name("smith")).to include(jane, john)
      expect(Patient.search_by_name("smith")).not_to include(mary)
    end
  end
