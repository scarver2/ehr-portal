...
RSpec.describe Diagnosis do
  ...
  describe 'enums' do
    it 'defines status values' do
      expect(described_class.statuses.keys).to match_array(%w[active resolved chronic ruled_out])
    end
  ...
  describe 'scopes' do
    ...
    it '.active filters active diagnoses' do
      expect(described_class.active).to include(active_dx)
      expect(described_class.active).not_to include(resolved_dx)
    end

    it '.chronic filters chronic diagnoses' do
      expect(described_class.chronic).to contain_exactly(chronic_dx)
    end

    it '.recent orders by diagnosed_at descending' do
      expect(described_class.recent.first).not_to eq(resolved_dx)
    end

    it '.by_code filters by ICD-10 code' do
      expect(described_class.by_code('I10')).to contain_exactly(chronic_dx)
    end

    it '.by_code returns empty when code does not match' do
      expect(described_class.by_code('Z99.99')).to be_empty
    end
  end

  describe '.ransackable_attributes' do
    it 'includes searchable fields' do
      expect(described_class.ransackable_attributes).to include('icd10_code', 'description', 'status', 'diagnosed_at')
    end
  end

  describe '.ransackable_associations' do
    it 'includes associated models' do
      expect(described_class.ransackable_associations).to include('encounter')
    end
  end
end
