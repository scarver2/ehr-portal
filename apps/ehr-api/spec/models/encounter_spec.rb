...
RSpec.describe Encounter do
  ...
  describe 'enums' do
    it 'defines encounter_type values' do
      expect(described_class.encounter_types.keys).to match_array(
  ...
  describe 'scopes' do
    ...
    it '.recent orders by encountered_at descending' do
      expect(described_class.recent.first).to eq(new_encounter)
    end

    it '.completed filters by completed status' do
      expect(described_class.completed).to contain_exactly(old_encounter)
    end

    it '.for_patient filters by patient' do
      expect(described_class.for_patient(new_encounter.patient)).to include(new_encounter)
      expect(described_class.for_patient(new_encounter.patient)).not_to include(old_encounter)
    end

    it '.for_provider filters by provider' do
      expect(described_class.for_provider(new_encounter.provider)).to include(new_encounter)
      expect(described_class.for_provider(new_encounter.provider)).not_to include(old_encounter)
    end
  end

  describe '.ransackable_attributes' do
    it 'includes key searchable fields' do
      expect(described_class.ransackable_attributes).to include('status', 'encounter_type', 'encountered_at')
    end
  end

  describe '.ransackable_associations' do
    it 'includes associated models' do
      expect(described_class.ransackable_associations).to include('patient', 'provider', 'vitals', 'diagnoses')
    end
  end
end
