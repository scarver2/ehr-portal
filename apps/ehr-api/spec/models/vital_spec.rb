...
RSpec.describe Vital do
  ...
  describe 'enums' do
    it 'defines all vital_type values' do
      expect(described_class.vital_types.keys).to match_array(
  ...
  describe 'UNITS constant' do
    it 'defines a unit for every vital type' do
      described_class.vital_types.each_key do |type|
  ...
  describe 'scopes' do
    ...
    it '.recent orders by observed_at descending' do
      expect(described_class.recent.first).to eq(hr_vital)
    end

    it '.by_type filters by vital_type' do
      expect(described_class.by_type(:blood_pressure)).to contain_exactly(bp_vital)
    end
  end

  describe '.ransackable_attributes' do
    it 'includes searchable fields' do
      expect(described_class.ransackable_attributes).to include('vital_type', 'value', 'observed_at', 'encounter_id')
    end
  end
