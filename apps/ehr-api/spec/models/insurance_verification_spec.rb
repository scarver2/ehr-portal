...
RSpec.describe InsuranceVerification do
  ...
  describe '#broadcast!' do
    ...
    context 'when Redis is unavailable' do
      it 'gracefully handles Errno::ECONNREFUSED' do
        verification = create(:insurance_verification, :verified)
        allow(InsuranceVerificationChannel).to receive(:broadcast_to).and_raise(Errno::ECONNREFUSED,
                                                                                'Connection refused')
      ...
      it 'gracefully handles RedisClient::CannotConnectError' do
        verification = create(:insurance_verification, :verified)
        allow(InsuranceVerificationChannel).to receive(:broadcast_to).and_raise(RedisClient::CannotConnectError,
                                                                                'Cannot connect')
        expect { verification.broadcast! }.not_to raise_error
      end
    end
