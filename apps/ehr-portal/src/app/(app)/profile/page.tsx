  ...
  return (
    <div className="p-8 max-w-3xl animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[0, 1, 2].map((i) => (
          <div key={i} className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex items-center gap-3">
    ...
    <div className="p-8 max-w-3xl">
      <div className="relative bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6 overflow-hidden">
        <div className="absolute right-4 top-4 opacity-[0.07] pointer-events-none select-none">
          <Image src="/icons/stethoscope.png" alt="" width={120} height={120} className="w-28 h-28" />
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-slate-800">{provider ? uniquePatients.length : "—"}</p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-slate-800">{provider ? provider.encounters.length : "—"}</p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex items-center gap-3">
            <Image src="/icons/stethoscope.png" alt="Specialty" width={36} height={36} className="shrink-0" />
            <div>
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (mounted && token === null) router.push("/")
  }, [mounted, token, router])

  // Server and client first-paint both return null — no hydration mismatch possible
